SELECT DISTINCT 
    ...
    (
        ...
        CASE 
            ...
            WHEN table_schema ILIKE '%_pt360_%' AND (table_name NOT ILIKE '%_claims%' AND table_schema NOT ILIKE '%_open_%') THEN '''Patient360-Claims'''
            WHEN table_schema ILIKE '%_gn360_%' AND (table_name ILIKE '%_claims%' OR table_schema ILIKE '%_open_%') THEN '''Genome360-Claims'''
            WHEN table_schema ILIKE '%_gn360_%' AND (table_name NOT ILIKE '%_claims%' AND table_schema NOT ILIKE '%_open_%') THEN '''Genome360-Claims'''
            ELSE '''' || table_schema || ''''
        END || ' AS delivery_type, ' ||
        CASE 
            WHEN table_schema ILIKE '%_pt360%Analytical%Layer%' THEN '''Patient360-Analytical Layer'''
        ...
        CASE 
            ...
            WHEN table_schema ILIKE '%_rcc%' OR table_name ILIKE '%_sclc_%' THEN '''Renal Cell Carcinoma (RCC)'''
            ELSE '''' || table_name || ''''
        END || ' AS Indication, ' ||
